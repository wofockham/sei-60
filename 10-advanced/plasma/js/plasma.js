...
const imgData = ctx.createImageData(width, height); // A typed array for fast access to the RGB of indivial pixels

// Math.sin is slow: Precalculate sine values between 0 and 255
const sineTable = []; // TODO: Make this faster by using a clamped array
for (let i = 0; i <= 255; i++) {
    sineTable[i] = Math.round(((Math.sin( i * 2 * Math.PI / 255) * 255) + 255) / 2); // Sorry about this
}
...
const paletteFactory = (colors) => {
    ...
    for (let i = 0; i < colors.length; i++) {
        const j = (i+1) % colors.length; // What does this even do?
...
const fill = (phase) => {
    for (let x = 0; x < width; x++) {
        for (let y = 0; y < height; y++) {
            ...
            let shade = Math.floor(shade1 + shade2 + shade3) % 256;

            // My god, why have you forsaken me?
            while (palette[shade] === undefined) {
                shade++;
            }
